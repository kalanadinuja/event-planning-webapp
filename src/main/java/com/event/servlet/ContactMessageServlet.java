...
import com.event.service.ContactMessageService;
...
public class ContactMessageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action == null) {
            ...
            request.getRequestDispatcher("contact/ManageContactIndex.jsp").forward(request, response);
        } else if ("edit".equals(action)) {
            int id = Integer.parseInt(request.getParameter("id"));
            ContactMessage message = contactMessageService.getContactMessage(id);
            request.setAttribute("message", message);
            request.getRequestDispatcher("contact/ManageContactEdit.jsp").forward(request, response);
